...
usage() {
  cat <<'EOF'
Usage:
  run_sssp_partition_bench.sh [options]
...
Options:
  ...
  --output-dir PATH    Base directory for logs and reports.
                       Default: ./benchmark_outputs/sssp_partition
  --csv PATH           Output CSV path.
                       Default: <output-dir>/sssp_partition_results.csv
...
OUTPUT_DIR="${OUTPUT_DIR:-./benchmark_outputs/sssp_partition}"
...
if [[ -z "$CSV_OUTPUT" ]]; then
  CSV_OUTPUT="${OUTPUT_DIR}/sssp_partition_results.csv"
fi
...
run_one_case() {
  ...
  local log_path="${OUTPUT_DIR}/logs/SSSP_${dataset_tag}_${ratio_pct}_${method_tag}.log"
