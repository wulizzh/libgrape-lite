...
class BasicFragmentLoader : public BasicFragmentLoaderBase<FRAG_T> {
 ...
 public:
  ...
  void AddVertex(const oid_t& id, const vdata_t& data) override {
    ...
    vprivacy_.emplace_back(0);
  }

  void AddVertex(const oid_t& id, const vdata_t& data, const int32_t& v_p) {
    vertices_.emplace_back(id);
    vdata_.emplace_back(data);
    vprivacy_.emplace_back(v_p);
  }

  void StoreOverride(const oid_t& id, const fid_t& fid) {
    overrides_.emplace_back(id, fid);
  }
  ...
  void ConstructVertices() override {
    ...
    // Apply overrides
    for(auto& p : overrides_) {
      partitioner->SetPartitionId(p.first, p.second);
    }

    std::vector<std::vector<oid_t>> local_vertices_id;
    std::vector<std::vector<vdata_t>> local_vertices_data;
    std::vector<std::vector<int32_t>> local_vertices_privacy;
    this->ShuffleVertexData(vertices_, vdata_, vprivacy_, local_vertices_id,
                            local_vertices_data, local_vertices_privacy, *partitioner);
    ...
    for (size_t buf_i = 0; buf_i < local_vertices_id.size(); ++buf_i) {
      ...
      std::vector<int32_t>& local_vprivacy = local_vertices_privacy[buf_i];
      ...
      for (size_t i = 0; i < local_vertices_num; ++i) {
        ...
        if (vertex_map_->GetGid(local_vertices[i], gid)) {
          processed_vertices_.emplace_back(gid, std::move(local_vdata[i]),
                                           static_cast<bool>(local_vprivacy[i]));
        }
      }
    }
  ...
  void AddEdge(const oid_t& src, const oid_t& dst,
    ...
    if (vertex_map_->GetGid(src, src_gid) &&
      ...
      edges_to_frag_[src_fid].Emplace(src_gid, dst_gid, data, 0);
      if (src_fid != dst_fid) {
        edges_to_frag_[dst_fid].Emplace(src_gid, dst_gid, data, 0);
      }
    }
  }
  ...
  void AddEdge(const oid_t& src, const oid_t& dst, const edata_t& data,
    ...
    if (vertex_map_->GetGid(src, src_gid) &&
      ...
      fid_t src_fid = id_parser_.get_fragment_id(src_gid);
      fid_t dst_fid = id_parser_.get_fragment_id(dst_gid);
      edges_to_frag_[src_fid].Emplace(src_gid, dst_gid, data, e_p);
      if (src_fid != dst_fid) {
        edges_to_frag_[dst_fid].Emplace(src_gid, dst_gid, data, e_p);
      }
    }
  }
  ...
  void ConstructFragment(std::shared_ptr<fragment_t>& fragment) override {
    ...
    for (auto& buffers : got_edges_) {
      foreach_rval(buffers,
                   [&processed_edges](vid_t&& src, vid_t&& dst, edata_t&& data,
                                      int32_t&& e_p) {
                     processed_edges.emplace_back(src, dst, std::move(data),
                                                  static_cast<bool>(e_p));
                   });
    }
 ...
 private:
  ...
  std::vector<int32_t> vprivacy_;

  std::vector<std::pair<oid_t, fid_t>> overrides_;

  std::vector<internal::Vertex<vid_t, vdata_t>> processed_vertices_;

  std::unique_ptr<VertexMap<oid_t, vid_t>> vertex_map_;

  std::vector<ShuffleOut<vid_t, vid_t, edata_t, int32_t>> edges_to_frag_;
  std::thread edge_recv_thread_;
  bool recv_thread_running_;

  std::vector<ShuffleBufferTuple<vid_t, vid_t, edata_t, int32_t>> got_edges_;
