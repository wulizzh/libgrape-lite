...
class HashPartitioner : public IPartitioner<OID_T> {
 public:
  ...
  fid_t GetPartitionId(const internal_oid_t& oid) const override {
    auto iter = override_map_.find(OID_T(oid));
 ...
 private:
  HASH_T hash_;
  fid_t fnum_;
  ska::flat_hash_map<OID_T, fid_t> override_map_;
};
...
class SegmentedPartitioner : public IPartitioner<OID_T> {
 ...
 public:
  ...
  fid_t GetPartitionId(const internal_oid_t& oid) const override {
    auto iter_map = override_map_.find(OID_T(oid));
    if (iter_map != override_map_.end()) {
